...
#Je vais faire un decoupage en 3 dans ce script et un en 5 ou 6 dans un autre script.
...
seq.part <- factor(
	seq.part,
	labels=c("1.Ass-Sen", "2.mono-mandat", "3.Ass-Congr�s")
)

# 1ere tentative de typologie - sequences en tapis :
png(file.path(seq.folder, "Typologie_3.png"))	# cree un fichier contenant le graphique
seqdplot(
	sd,
	group=seq.part,
	border=NA,
...
png(file.path(seq.folder, "sequences_classes_3.png"), width = 1024)
seqIplot(
	sd,
	group=seq.part,
)
...
png(file.path(seq.folder, "Typologie_a-histo_3.png"), width = 1024)	# cree un fichier contenant le graphique
seqdplot(
	sd.left,
	group=seq.part,
	border=NA,
...
png(file.path(seq.folder, "sequences_classes_a-histo_3.png"), width = 1024)
seqIplot(
	sd.left,
	group=seq.part,
	xlab=NA,							# n'affiche pas le nom de l'axe x
...
png(file.path(seq.folder, "repartition_etats_classes_3.png"))
seqmtplot(
	sd,
	group=seq.part,
	border=NA
...
png(file.path(seq.folder, "repartition_etats_classes_a-histo_3.png"))
seqmtplot(
	sd.left,
	group=seq.part,
	border=NA,
...
medoid.ids <- disscenter(as.dist(seq.om), group=seq.part, medoids.index="first")
print(medoid.ids)
...
#Pourcentage
100*(table (seq.part))/length(seq.part)
...
dist.to.centers <- disscenter(as.dist(seq.om), group=seq.part)
round(aggregate(dist.to.centers, list(seq.part),mean)[,-1],1)
...
png(file.path(seq.folder, "entropie_transversale_3.png"))
seqHtplot(sd, group=seq.part)
dev.off()
...
#Ensemble des relations entre attributs de repr.seq et les trajectoires :
#Attention, Female et Race ne sont pas des attributs booleens, je dois donc attribuer une valeur a ces attributs
# Par ex, je peux choisir de regarder avec Party = D / Race = B. 

#catdesc(factor(seq.part), repr.seq[seq.ids,"Female"] & repr.seq[seq.ids,"Race"]=="W" & repr.seq[seq.ids,"Party"]=="D", min.phi=0.0)
...
#############################################
######## ANALYSE DES VARIANCES ET ARBRES DE DECISION
#####################################################

#Ensuite, je cherche a analyser la variance selon le facteur :
#Variance selon le sexe : ERREUR
#dissassoc(sd,
         # repr.seq[, "Female"])
#ETAPE NECESSAIRE POUR REALISER L ARBRE GRAPHIQUE. 
...
png(file.path(seq.folder, "distance_class.png"))
...
points(
  ...
  col=seq.part							# couleur(s) utilisee pour dessiner les points
...
png(file.path(seq.folder, "distance_class_sex.png"))
plot(
  mds,
  type="n",
...
png(file.path(seq.folder, "distance_sex.png"))
plot(
  mds,
  type="n",
  xlab="Dimension 1", ylab="Dimension 2"
...
png(file.path(seq.folder, "distance_class_race.png"))
plot(
  mds,
  type="n",
  xlab="Dimension 1", ylab="Dimension 2"
...
png(file.path(seq.folder, "distance_class_party.png"))
plot(
  mds,
  type="n",
  xlab="Dimension 1", ylab="Dimension 2"
...
###############
# Je realise enfin les graph pour le party:
# Version modifiee, avec le party a la place de la classe:
png(file.path(seq.folder, "distance_party.png"))
plot(
  mds,
  type="n",
  xlab="Dimension 1", ylab="Dimension 2"
)
# j'utilise le symbole rouge pour les R et le bleu pour les D
# (cliche, je sais, mais ca permet de comprendre tout de suite le graphique !)
cols <- rep("BLUE", length(repr.seq[seq.ids,"Party"]))
cols[repr.seq[seq.ids,"Party"]=="R"]  <- "RED"
points(
  mds,
  pch=20,
  col=cols
